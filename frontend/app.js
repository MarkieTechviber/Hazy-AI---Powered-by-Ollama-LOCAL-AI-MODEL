...
const CODE_LANGUAGES = [
  ...
  { label: 'C++',            value: 'cpp',         ext: 'cpp',   icon: '⚙️' },
  ...
  { label: 'PowerShell',     value: 'powershell',  ext: 'ps1',   icon: '🖥️' },
  { label: 'SQL',            value: 'sql',         ext: 'sql',   icon: '🗄️' },
  { label: 'Assembly',       value: 'asm',         ext: 'asm',   icon: '⚙️' },
  { label: 'MATLAB',         value: 'matlab',      ext: 'm',     icon: '📐' },
  { label: 'Fortran',        value: 'fortran',     ext: 'f90',   icon: '🏛️' },
...
const SCENARIO_SETTINGS = [
  { id: 'school',    emoji: '🏫', label: 'School / Campus' },
  { id: 'office',    emoji: '🏢', label: 'Office / Work' },
  { id: 'cafe',      emoji: '☕', label: 'Café / Coffee Shop' },
  { id: 'home',      emoji: '🏠', label: 'Home / Neighborhood' },
  { id: 'fantasy',   emoji: '⚔️', label: 'Fantasy World' },
  { id: 'scifi',     emoji: '🚀', label: 'Sci-Fi / Future' },
  { id: 'hospital',  emoji: '🏥', label: 'Hospital / Recovery' },
  { id: 'travel',    emoji: '✈️', label: 'Traveling / Adventure' },
  { id: 'online',    emoji: '💻', label: 'Online / Social Media' },
...
function updatePersonaBadge() {
  ...
  if (STATE.personaEnabled) {
    ...
    badge.textContent = `${preset?.emoji || '🎭'} ${STATE.personaName}`;
...
function renderPresetScenarioGrid() {
  ...
  grid.innerHTML = SCENARIO_PRESETS.map(p => `
    <button class="preset-scenario-card ${STATE.scenarioDesc === p.scenarioDesc && p.id !== 'custom' ? 'selected' : ''}"
      data-id="${p.id}">
      <span class="preset-scenario-emoji">${p.emoji}</span>
...
function setMode(mode) {
  ...
  el.modeIndicator.textContent = mode === 'build' ? '🌐 Website Builder mode'
    : mode === 'code' ? '💻 Code Builder mode'
    : '💬 Chat mode';
...
function savePersona() {
  ...
  if (STATE.personaEnabled) {
    const preset = PERSONA_PRESETS[STATE.personaRelation];
    showToast(`${preset?.emoji || '🎭'} ${STATE.personaName} — starting scene…`, 'success');
