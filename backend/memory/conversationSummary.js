function summarizeConversation(messages = []) {
  ...
  const assistantMoves = recent.filter((message) => message.role === "assistant").slice(-2).map((message) => message.content);

  return {
    summary: [
      userGoals.length ? `Recent user goals: ${userGoals.join(" | ")}` : null,
      assistantMoves.length ? `Recent assistant help: ${assistantMoves.join(" | ")}` : null
    ].filter(Boolean).join("\n")
  };
}
