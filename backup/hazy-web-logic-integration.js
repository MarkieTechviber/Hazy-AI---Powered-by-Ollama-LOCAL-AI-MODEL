 ...
 * pattern).
 *
 * NOTE (post-enforcement): The authoritative runtime with full AI Workspace
 * Operating Rules redirect (wsKinds to #hazyWorkspaceHost, no bubble pollution,
 * live open-panel only for file cards) is the frontend/ copy. This root copy
 * is a basic stub kept for reference/dev; if loaded directly it now has a
 * minimal guard (see end of IIFE) to skip ws cards (preventing transcript
 * code walls per Code Visibility rules). Keep frontend/ as source of truth
 * for future ws logic; do not rely on root for enforcement.
 *
...
<details class="hazy-structured-card ${meta.cssClass || ''}" data-kind="${meta.kind}" data-id="${escapeHtmlSafe(entry.id || entry.tool_id || entry.cite_id || '')}" data-status="${status}"${openAttr}>
  ...
  // Minimal guard (root stub only; full ws redirect/enforcement in frontend/ authoritative copy):
  // Skips ws cards (file-panel etc) to host or entirely (never to bubbles) to uphold Code Visibility.
  (function installRootGuard(){ 
    const _r = window.applyHazyStructuredUpdate; 
    const w = ['file-panel','file','project-workspace','workspace','project-thinking','thinking'];
    window.applyHazyStructuredUpdate = function(c,e){ 
      if(e && w.includes(e.kind)){ 
        const h=document.getElementById('hazyWorkspaceHost')||document.getElementById('threadProjectWorkspace'); 
        if(h){try{h.insertAdjacentHTML('beforeend','');}catch(_){}} 
        return true;
      } 
      return _r?_r(c,e):false; 
    };
  })();
})();